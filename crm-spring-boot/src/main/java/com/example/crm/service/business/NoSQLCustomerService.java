...
public class NoSQLCustomerService implements CustomerService {
	...
	private static final Supplier<CustomerNotFoundException> customerNotFoundExceptionSupplier = () -> new CustomerNotFoundException(
			"Cannot find the customer", 1000, "821b0947-4758-49da-a0f6-a08fad807016");
	...
	public CustomerResponse findById(String identity) {
		return modelMapper.map(customerRepository.findById(identity).orElseThrow(customerNotFoundExceptionSupplier),
				CustomerResponse.class);
	}

	@Override
	public List<CustomerResponse> findAll(int pageNo, int pageSize) {
		return customerRepository.findAll(PageRequest.of(pageNo, pageSize)).stream()
				.map(customer -> modelMapper.map(customer, CustomerResponse.class))
				.sorted(Comparator.comparing(CustomerResponse::getFullname)).toList();
	}
	...
	public UpdateCustomerResponse updateCustomer(String identity, UpdateCustomerRequest request) {
		var customer = customerRepository.findById(identity).orElseThrow(customerNotFoundExceptionSupplier);
	...
	public UpdateCustomerResponse patchCustomer(String identity, Map<String, Object> request) {
		...
		return modelMapper.map(customerRepository.save(customer), UpdateCustomerResponse.class);
	}
